...
class ThanhvienDashboardController extends Controller
{
    ...
    public function bieumau($type)
    {
        $user = Auth::user();
        if ($type == 1)
        $bieumau = Bieumau1::where('user_id', $user->id)->where('reporter_year', $user->bieumauYearFinish())->first();
        else if ($type == 2)
        $bieumau = Bieumau2::where('user_id', $user->id)->where('reporter_year', $user->bieumauYearFinish())->first();
        else if ($type == 3)
        $bieumau = Bieumau3::where('user_id', $user->id)->where('reporter_year', $user->bieumauYearFinish())->first();
        else if ($type == 4)
        $bieumau = Bieumau4::where('user_id', $user->id)->where('reporter_year', $user->bieumauYearFinish())->first();
        else if ($type == 5)
        $bieumau = Bieumau5::where('user_id', $user->id)->where('reporter_year', $user->bieumauYearFinish())->first();
        else if ($type == 6)
